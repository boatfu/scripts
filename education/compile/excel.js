...
// 校验表头
function compareHeader(mapA, mapB) {
    const arrA = Object.keys(mapA);
    const arrB = Object.keys(mapB);
...
function sumUp(oriWorkbookName, dirName, outputName) {
    ...
    for (const fileName of files) {
        ...
        workbooks.push(xlsx.readFile(dirName + '/' + fileName));
    }
    ...
    for (const sheetName of SheetNames) {
        const worksheets = [];
        const { headerMap } = getHeaderInfo(oriWorkbook.Sheets[sheetName]);
        for (const [i, workbook] of Object.entries(workbooks)) {
            if (!workbook.Sheets[sheetName]) {
                console.log(
                    `文件 ${
                        files[workbooks.indexOf(workbook)]
                    } 出错, 找不到${sheetName}`
                );
                return;
            }
            const { headerMap: subHeaderMap } = getHeaderInfo(
                workbook.Sheets[sheetName]
            );

            if (compareHeader(headerMap, subHeaderMap) !== true) {
                const [oriName, subName] = compareHeader(
                    headerMap,
                    subHeaderMap
                );
                console.log(
                    `文件 "${files[i]}"中"${sheetName}" 的表头"${subName}"与模板的"${oriName}"不匹配, 请重新检查`
                );
                return;
            }
